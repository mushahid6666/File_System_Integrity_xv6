...
//		cprintf("checksum = %x,addr = %x\n",checksum,addr);
    ...
    //Checksum if checksum and current xor correct

    if(ip->type == T_CHECKED){
//    	cprintf("tot = %d m =%d , off =%d \n",tot,m,off);
    		for(k=0;k<512;k++){
    				xor ^= bp->data[k];
//    				cprintf("%x",bp->data[k]);
    		}
//    		cprintf("xor = %x,size = %d",xor,ip->size);
    		if (checksum != xor) {
...
//    cprintf("\n print and xor calc after memove");
//    for(k=0;k<512;k++){
//        				xor ^= bp->data[k];
//        				cprintf("%x",bp->data[k]);
//	}
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    ...
    uint k=0;
    if(ip->type == T_CHECKED){
		//Calculate the checksum of that Data block
		uint bn = off/BSIZE;
		if( bn < NDIRECT) {
			uint prev_xor;
			prev_xor = (ip->addrs[off/BSIZE] & 0xff000000) >>24;
			for(k=off%BSIZE;k<(off%BSIZE+m);k++){
				prev_xor ^= bp->data[k];
			}
			prev_xor = prev_xor <<24;
			addr = addr & 0x00ffffff;
			ip->addrs[off/BSIZE] = addr | prev_xor;
			//cprintf("pre_xor left shift = %x, add = %x, ip->addrs[off/BSIZE] = %x\n",prev_xor, addr, ip->addrs[off/BSIZE]);
//			ip->addrs[off/BSIZE] = ((addr & 0x00ffffff)| (prev_xor <<24));
//			iupdate(ip);
//			cprintf("off = %d checksum = %x, add = %x\n",off,prev_xor,ip->addrs[off/BSIZE]);
		}
		else{
			bn -= NDIRECT;
			if(bn < NINDIRECT){
				uint addr_temp = ip->addrs[NDIRECT];
				struct buf *bp_temp = bread(ip->dev, addr_temp);
				uint *a_temp = (uint*)bp_temp->data;
				uint prev_xor;
				prev_xor = (a_temp[bn] & 0xff000000) >>24;
				for(k=off%BSIZE;k<(off%BSIZE+m);k++){

					prev_xor ^= (uint)(bp->data[k]);
		//				cprintf("%x",bp->data[k]);
				}
		//		cprintf("indirect checksum = %x, add = %x\n",prev_xor,addr);
				a_temp[bn] = ((addr & 0x00ffffff)| (prev_xor <<24));
				bwrite(bp_temp);
				brelse(bp_temp);
			}
		}
